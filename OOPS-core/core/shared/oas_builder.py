...
class OASBuilder:
    ...
    def upgrade_apidoc(self: Self, silent: bool = False) -> None:
        ...
        # jdk 17 and older write files in the platform default charset, which on a windows host is cp1252 —
        # codegen then emits an en-dash as 0x96 and the utf-8 read below dies on it. LANG alone does not
        # reach the jvm, so set the charset on the jvm itself

        command = ['java', '-Dfile.encoding=UTF-8', '-jar', f'{os.path.dirname(__file__)}/{OAS_UPGRADE_CODEGEN}', 'generate', '-l', 'openapi']
        ...
        result = subprocess.run(chr(32).join(command), cwd=f'{self.__temp_upgrade}/upgrade', check=False, timeout=60,
            ...
            # passing a bare dict here replaces the whole environment and wipes PATH, so the jvm is never
            # found ("'java' is not recognized") — merge onto os.environ instead

            shell=True, encoding='utf-8', env={**os.environ, 'LANG': 'en_US.UTF-8'})

        assert not (not silent and print(result.stdout.strip()))

        assert not (not silent and print(result.stderr.strip()))

        # errors='replace' as a second line of defence behind -Dfile.encoding above: a single stray byte in a
        # description is not worth losing the whole upgraded document to a UnicodeDecodeError

        with open(f'{base}/openapi.json', 'r', encoding='utf-8', errors='replace') as file:
