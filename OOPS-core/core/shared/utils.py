...
from pydantic import ValidationError  # not re-exported by core.libraries
...
def safe_validate(schema: type, payload: Any) -> Any:

    # agno substitutes a plain string (e.g. 'Unknown model error') for the structured output whenever the
    # provider errors out, so validating it raises and — inside an asyncio.gather batch — aborts the whole
    # run. callers get None instead and decide whether to retry or skip.
