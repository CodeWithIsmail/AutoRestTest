...
from core.shared.rate_limiter import RateLimiter


class LLMFactory:

    __shared_rate_limiter = RateLimiter(LLM_RATE_LIMIT_RPM)
    ...
    __extra_body: dict[str, Any] | None
    ...
    def __init__(
        ...
        # merged into the JSON body rather than the query string, which is where provider-specific knobs
        # live — e.g. {'chat_template_kwargs': {'thinking': False}} to stop a reasoning model from spending
        # its whole completion budget thinking (measured: 1069 tokens/25s -> 26 tokens/0.7s on nemotron)

        self.__extra_body = extra_body if extra_body else None
    ...
    def build(self: Self, store: str) -> Model:
        ...
        async def throttle_request(request: httpx.Request) -> None:

            await self.__shared_rate_limiter.acquire()

        # ------------------------------------------------

        async def collapse_parallel_tool_calls(request: httpx.Request) -> None:

            # NIM's llama-3.1-8b template rejects any history holding an assistant turn with more than one
            # tool_call ('This model only supports single tool-calls at once!', HTTP 500). the model emits
            # them anyway and the provider ignores parallel_tool_calls=false, so the only place left to fix
            # it is the outgoing request: keep the first call and drop the orphaned tool results.
            ...
            for message in messages:
                ...
                if isinstance(calls, list) and len(calls) > 1:
                    ...
                    orphans.update(i.get('id') for i in calls[1:])
            ...
            payload['messages'] = [
                i for i in messages
                if not (isinstance(i, dict) and i.get('role') == 'tool' and i.get('tool_call_id') in orphans)
            ]

            content = json.dumps(payload, ensure_ascii=False).encode('utf-8')

            # the transport sends request.stream, not request.content — all three have to agree or h11
            # raises 'Too much data for declared Content-Length'

            request._content = content  # pylint: disable=protected-access

            request.stream = httpx._content.ByteStream(content)  # pylint: disable=protected-access
        ...
        # the collapse hook exists for one broken template and rewrites every outgoing request. models that
        # handle parallel tool calls correctly lose real calls to it, so only arm it where it is needed

        request_hooks = [throttle_request]

        if 'llama' in self.__model_name.lower():

            request_hooks.insert(0, collapse_parallel_tool_calls)
        ...
        return os.makedirs(store, exist_ok=True) or OpenAIChat(
            ...
            http_client=httpx.AsyncClient(
                ...
                event_hooks={'request': request_hooks, 'response': [log_response]}
            )
        )
