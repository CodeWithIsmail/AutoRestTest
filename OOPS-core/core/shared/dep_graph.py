...
class DependencyGraph:
    ...
    def add_relation(self: Self, entry: Entry, target: str) -> None:
        ...
        if all(i != entry.handler for i in self.__file_to_entry.get(target, [])):
            ...
            # carry the parent's mount prefix across. dropping it (path='') leaves endpoint-method-extract
            # with no prefix to work from for routers mounted via app.use('/api/v1/users', userRouter), and
            # the model then invents one — '/api/categories' instead of '/api/v1/categories'

            self.add_entry(DependencyGraph.Entry(file=target, path=entry.path, feat='', handler=entry.handler, tag='local'))
