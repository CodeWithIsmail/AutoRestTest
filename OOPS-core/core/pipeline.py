...
class MainPipeline:
    ...
    def __init__(self: Self, title: str, version: str, project: str, log_base: str, upg_base: str, config: Config):
        ...
        # several steps assert these exist rather than creating them, so a first run against a project whose
        # log directory has not been made by hand dies on an assertion right after step one

        os.makedirs(log_base, exist_ok=True)

        os.makedirs(upg_base, exist_ok=True)
    ...
    async def run_endpoint_method_extract(self: Self) -> None:
        ...
        # batch_run_lite yields None for any entry that burned all its retries. keeping those would crash
        # every downstream consumer (add_openapi_operation on v.methods, dump_states on v.model_dump), so
        # drop them here — one bad file costs its own endpoints, not the whole run
    ...
    async def run_swagger_generation(self: Self) -> None:
        ...
        self.__swagger_components = {k: v for k, v in zip(tasks, await SwaggerGenerationAgent.batch_run_lite(
            ...
            # this stage is two llm conversations per operation and is where long runs die. cache each one
            # next to its logs so a rerun resumes from the endpoints already generated instead of paying
            # for the whole batch again — delete the directory to force a clean regeneration

            cache=f'{self.__log_base}/swagger-generation/_cache'
        )) if v is not None}
