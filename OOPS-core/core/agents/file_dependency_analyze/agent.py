...
class FileDependencyAnalyzeAgent:
    ...
    async def __batch_run(
        ...
        # one file raising takes the whole gather down with it and loses every other file in the batch, so
        # isolate failures into None — the callers already drop those the same way they drop a file that
        # burned all of its retries

        results = await asyncio.gather(*[solve(semaphore, *args) for args in zip(
        ...
        return [None if isinstance(i, BaseException) else i for i in results]
    ...
    async def run(
        ...
        while retries and (output is None or errors):
            ...
            # the parser model intermittently hands back raw text instead of the schema (seen when the
            # provider is rate limiting) — count it as a spent attempt and retry, rather than raising and
            # taking down the whole asyncio.gather batch with it

            if not isinstance(content, (FileDependencyAnalyzeResult, FileDependencyAnalyzeRefine)):

                retries, output, errors = retries - 1, None, None

                continue

            retries, output = retries - 1, sorted({os.path.normpath(i) for i in content.files})

            errors = [i for i in output if i not in files]  # filter outputs using whitelist

            errors = [i for i in output if i not in files]  # filter outputs using whitelist

        # output stays None when every retry failed to parse — degrade to no dependencies for this file
        # instead of raising, so one bad file cannot abort the run

        rebuild = FileDependencyAnalyzeResult(files={i for i in (output or []) if i in files and i != file})
