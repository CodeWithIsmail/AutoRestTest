...
from core.shared import safe_validate
...
class APIEntryDetectionAgent:
    ...
    async def run(
        ...
        # when the provider errors (e.g. a 429 storm) agno yields a plain string such as 'Unknown model
        # error' in place of the schema — skip this file instead of raising and aborting the whole batch

        status = safe_validate(FileFilterStatus, getattr(result, 'content', None))

        if status is not None and status.contains_api_entries:
            ...
            while retries and (isinstance(extend, set) or extend):
                ...
                batch = safe_validate(APIEntryDetectionResultList, getattr(result, 'content', None))

                if batch is None:  # spent this attempt on a provider error, let the retry loop try again

                    extend = True

                    continue

                for i in batch.entry:
