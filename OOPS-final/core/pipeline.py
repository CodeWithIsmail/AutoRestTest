...
class MainPipeline:
    ...
    class Config(BaseModel, frozen=True, arbitrary_types_allowed=True):
        ...
        llm_extra_kwargs: dict[str, Any] | None = Field(default=None)
    ...
    async def pre_technology_analyze(self: Self) -> None:
        ...
        result = await TechnologyAnalyzeAgent.run(self.__file_handler, worker_model, parser_model, extra=self.__common_config.llm_extra_kwargs)
    ...
    async def run_api_entry_detection(self: Self) -> None:
        ...
        self.__api_entries = {k: v for k, v in zip(tasks, await APIEntryDetectionAgent.batch_run_lite(
            ...
            [i for i in tasks],
            extra=self.__common_config.llm_extra_kwargs
        ))}
