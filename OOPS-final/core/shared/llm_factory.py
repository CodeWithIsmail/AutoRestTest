from core.shared.rate_limiter import get_rate_limiter
...
class LLMFactory:
    ...
    def __init__(
        ...
        self.__rpm_limit = rpm_limit  # proactive requests-per-minute pacing, disabled unless set
    ...
    def build(self: Self, store: str) -> Model:
        ...
        limiter = get_rate_limiter(self.__model_name, self.__rpm_limit) if self.__rpm_limit else None

        async def pace_request(request: httpx.Request) -> None:

            if limiter is not None:

                await limiter.acquire()  # block until issuing another request stays within the configured rpm
        ...
        return os.makedirs(store, exist_ok=True) or OpenAIChat(
            ...
            # agno auto-registers every Toolkit method with requires_confirmation/external_execution
            # fields (defaulting to False, not None, so they always serialize into tools[].function),
            # but only strips them for provider in ("AIMLAPI", "Fireworks", "Nvidia") before sending;
            # strict OpenAI-compatible backends (e.g. Google's Gemini endpoint) reject these unknown
            # fields with a 400. 'provider' has no other effect (only used for this check and cosmetic
            # debug-log labels elsewhere in agno), so spoofing a whitelisted value is a safe workaround.
            provider='Fireworks',
            http_client=httpx.AsyncClient(
                ...
                event_hooks={'request': [pace_request], 'response': [log_response]}
            )
        )
