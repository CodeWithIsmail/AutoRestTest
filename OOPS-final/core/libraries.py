...
matplotlib.use('Agg')  # force a non-interactive backend; we only ever savefig() PNGs, never
                        # show a GUI, and the default TkAgg backend spawns Tk-backed figure
                        # objects that error out ("main thread is not in main loop") when
                        # garbage-collected outside Tk's own event loop, which never runs here
