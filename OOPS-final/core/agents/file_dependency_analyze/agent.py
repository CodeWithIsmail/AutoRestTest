...
class FileDependencyAnalyzeAgent:
    ...
    async def run(
        ...
        while retries and (output is None or errors):
            ...
            # normpath uses the OS-native separator, but FileHandler always keys files with forward
            # slashes (.as_posix()); without forcing '/' back here, every path fails the whitelist
            # check below on Windows, since the two candidates would never match syntactically.

            retries, output = retries - 1, sorted({os.path.normpath(i).replace(os.sep, '/') for i in getattr(result.content, 'files')})
